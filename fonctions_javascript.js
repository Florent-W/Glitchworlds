        ...
        $(function() { // Fonction autocompletion
            ...
            $('#' + idInput).catcomplete({
                    ...
                    select: function(event, ui) { // Quand on sélectionne un résultat, on va vers sa page
                        if (categorieRecherche != "Jeux" && categorieRecherche != "Plateformes" && categorieRecherche != "Genres" && categorieRecherche != "Langues") { // Si on est sur la page de création de news, pas besoin de redirigé quand on clique
                            if (ui.item.category == "Jeux") { // Vers page des jeux
                                document.location.href = "/jeu/" + ui.item.url + "-" + ui.item.id;
                            }
                            else if (ui.item.category == "Articles") { // Vers page des articles
                ...
                .data('custom-catcomplete')._renderItem = function(ul, item) { // On ajoute les images pour les résultats
                    if (item.category == "Jeux") { // Si c'est un jeu, on change la présentation et l'url
                        return $('<li></li>')
                            ...
                            .append('<div><img src="/Jeux/' + item.url + '/miniature/' + item.image + '" onerror="this.onerror=null; this.src=\'/1.jpg\';" class="img-fluid" style="width:30%; height: auto; max-width:50px; max-height:150px; border-style: none !important; background-color:transparent;">' + '     ' + item.value + '</div>') // image
                            .appendTo(ul)
                    } else if (item.category == "Articles") { // Si c'est un article, on change la présentation et l'url
                        return $('<li></li>')
                            .data('ui-item-autocomplete', item)
                            .append('<div><img src="/Articles/' + item.date + '/' + item.url + '/miniature/' + item.image + '" onerror="this.onerror=null; this.src=\'/1.jpg\';" class="img-fluid" style="width:30%; height: auto; max-width:50px; max-height:150px; background-color:transparent;">' + '     ' + item.value + '</div>') // image
                            .appendTo(ul)
                    } else if (item.category == "Plateformes") { // Si c'est une plateforme, on change la présentation
                        return $('<li></li>')
                            .data('ui-item-autocomplete', item)
                            .append('<div><img src="/images/plateformes/' + item.image + '" onerror="this.onerror=null; this.src=\'/1.jpg\';" class="img-fluid" style="width:10%; height: auto; max-width:50px; max-height:150px; background-color:transparent;">' + '     ' + item.value + '</div>') // image
                            .appendTo(ul)
                        ...
                        return $('<li></li>')
                            ...
                            .appendTo(ul)
                    } else if (item.category == "Langues") { // Si c'est une langue, on change la présentation
                        return $('<li></li>')
                            .data('ui-item-autocomplete', item)
                            .append('<div><img src="/images/langues/' + item.image + '" onerror="this.onerror=null; this.src=\'/1.jpg\';" class="img-fluid" style="width:10%; height: auto; max-width:50px; max-height:150px; background-color:transparent;">' + '     ' + item.value + '</div>') // image
                            .appendTo(ul)
                    }
                }
    ...
    // Fonction qui va empêcher de quitter la page
    function popupChangementDePage() {
        var formulaireSoumis = false;

        $(document).ready(function () {
            // Désactiver la boîte de dialogue lorsque le formulaire est soumis
            $('form').not('#form-recherche-header').submit(function () {
                formulaireSoumis = true;
            });

            // Activer la boîte de dialogue de sortie
            $(window).bind('beforeunload', function () {
                if (!formulaireSoumis) {
                    return 'Vous êtes sur le point de quitter cette page. Êtes-vous sûr ?';
                }
            });
        });
    }
    ...
    function inputPageRecherche() {
        $('#page').on('keypress', function(e) { // Si on est sur l'input
            if (e.which == 13) { // Si la touche entrée est pressé
                ...
                var rechercheValue = encodeURIComponent(new URLSearchParams(location.search).get('recherche'));
                var categorieValue = encodeURIComponent(new URLSearchParams(location.search).get('categorie'));
                var categorieJeuValue = encodeURIComponent(new URLSearchParams(location.search).get('categorie_jeu'));
                var plateformeValue = encodeURIComponent(new URLSearchParams(location.search).get('plateforme'));
                var genreValue = encodeURIComponent(new URLSearchParams(location.search).get('genre'));
                var langueValue = encodeURIComponent(new URLSearchParams(location.search).get('langue'));
                var favorisValue = encodeURIComponent(new URLSearchParams(location.search).get('favoris'));
                var triValue = encodeURIComponent(new URLSearchParams(location.search).get('tri'));

                var lien = "/recherche.php?recherche=";
                ...
                lien += "&categorie_jeu=";
                if (categorieJeuValue !== 'null') {
                     lien += categorieJeuValue;
                }

                lien += "&plateforme=";
                if (plateformeValue !== 'null') {
                     lien += plateformeValue;
                }
    ...
    // Va remplir un formulaire en fonction du template pour la création des articles ou autres qui prend en paramètre le nom d'un template que l'on veut, le champ sera rempli selon le template
    function ajoutTexteFormulaireTemplate(nomTemplate) {
        if($('#contenu').val() == "") { // On regarde si le champ est vide pour savoir si on peut appliquer le formulaire
            if(nomTemplate == "Glitch") { // Suivant le template, on insère un texte différent
                remplirChamp('[i]Veuillez insérer une description du glitch.[/i]\n\n[u]Jeux Compatibles[/u] :\n\n[u]Liste des étapes[/u] : [liste][elementliste]Première étape[/elementliste][elementliste]Deuxième étape[/elementliste][elementliste]Troisième étape[/elementliste][/liste]', 'contenu');
                // $('#template_group').hide() // On cache les templates après
            }
            else if(nomTemplate == "Presentation_Pokemon") {
                remplirChamp("[center]Veuillez insérer une description du jeu.[/center]\n[center][titre=h4][icone=histoir.png][/icone]Histoire :[/titre][/center]\n[center][titre=h4][icone=Important.png][/icone] Fonctionnalités :[/titre][/center]\n[center][titre=h4]Durée de vie :[/titre][/center]\n[center][titre=h4][icone=pokedex-kanto.png][/icone]Pokédex :[/titre][/center]\n [center][titre=h4][icone=iconemap.png][/icone] Région :[/titre][/center]\n[center][titre=h4]Personnages :[/titre][/center][center][titre=h4]Starters :[/titre][/center]\n[center][titre=h4]Langues :[/titre][/center][center][titre=h4]Plateformes :[/titre][/center]\n[center][titre=h4]Date de Sortie :[/titre][/center]\n[center][titre=h4][icone=iconecoupe.png][/icone]Crédits :[/titre][/center]\n[center][titre=h4][icone=iconecapture.png][/icone]Captures d'écrans :[/titre]\n[gallery][/gallery][/center]\n[center][titre=h4]Téléchargement :[/titre]\n[lien][/lien][texteLien][icone=iconetelechargement.png][/icone]Pokémon[/texteLien]", 'contenu');
                // $('#template_group').hide() // On cache les templates après
            }
        }
    }

    // Permet de cacher le menu des templates si il y a un contenu ou de le montrer si il y en a pas
    function montrerMenuTemplate() {
        $('#contenu').on('change', function() {
        if ($(this).val() != '') { // Si le contenu n'est pas vide, on doit cacher le menu des template pour éviter qu'ils effacent tout le contenu
            if ($('#template_group').is(':visible')) { // On regarde si le menu est visible et si oui, on doit le cacher
                $('#template_group').hide();
            }
        }
        else { // Si le contenu est vide et que le menu des template est caché, on le remet
            if ($('#template_group').is(':hidden')) { // On regarde si le menu est visible et si non, on doit le montré
                $('#template_group').show();
            }
        }
    });
    ...
    // Fonction qui va ajouter ou supprimer un favori
    function ajouterFavori($id_utilisateur, $id_jeu) {
        $.ajax('/ajouter_favori_requete.php', {
            data: { // On passe les valeurs
            ...
            console.log(reponse);
            if (reponse.success) {
                if ($('#btn_ajouter_favori').data('ui-tooltip')) {
                    $('#btn_ajouter_favori').tooltip('destroy');
                    $('#btn_ajouter_favori').removeAttr('title');
                }
                
                if (reponse.type.toLowerCase() === 'ajout') { 
                    $('#btn_ajouter_favori').one('mouseleave', function() {
                        // Supprimez la classe et ajouter lorsque l'utilisateur part du bouton
                        $('#btn_ajouter_favori').removeClass('etoile-non-favori');
                        $('#btn_ajouter_favori').addClass('etoile-favori');
                        $('#btn_ajouter_favori').attr('title', 'Retirer le jeu des Favoris');
                        $('#btn_ajouter_favori').tooltip();
                    });

                    $('#btn_ajouter_favori').css('color', 'yellow');
                } else if (reponse.type.toLowerCase() === 'retirer') {
                    $('#btn_ajouter_favori').one('mouseleave', function() {
                        // Supprimez la classe et ajouter lorsque l'utilisateur part du bouton
                        $('#btn_ajouter_favori').removeClass('etoile-favori');
                        $('#btn_ajouter_favori').addClass('etoile-non-favori');
                        $('#btn_ajouter_favori').attr('title', 'Ajouter le jeu en Favoris');
                        $('#btn_ajouter_favori').tooltip();
                    });

                    $('#btn_ajouter_favori').css('color', 'white');
                }
            }
        });
    }
