...
<h4 style="margin-bottom: 20px;" class="d-flex justify-content-center">Liste des Glitchs de <?php echo $nom_jeu; ?> :</h4>
<ul class="list-group" style="margin-bottom: 20px; top:100px">

    <?php
    $reponse = $bdd->prepare('SELECT COUNT(*) as nb_article FROM article INNER JOIN jeu ON article.id_jeu = jeu.id WHERE article.nom_categorie = "Glitch" AND jeu.nom = :nom_jeu AND article.titre LIKE :article'); // Nombre de glitchs trouvés, si aucun, on n'affichera pas
    ...
    if ($nbArticleTrouver > 0) {
        ...
        $positionArticle = 0; // On va voir la place du jeu et une fois sur deux, il sera en couleur
        while ($donnees = $reponse->fetch()) {
    ?>
            <!-- Liste glitch -->
            <?php if ($positionArticle % 2 == 0) { // Un glitch sur deux sera en couleur
            ?>
                ...
                <?php }
                    $positionArticle++; // On augmente la position du glitch vu qu'on change de glitch
                }
