...
if (isset($_POST['nom'])) {
    ...
    */
} else if (isset($_FILES['background_site']['name'])) {
    $categorie_article = "utilisateurs/" . $_SESSION['id'] . '/'; // A ranger dans le dossier utilisateurs
    //  $url = EncodageTitreEnUrl($_POST['titre']);
    $titre_article = '';
    $nom_image = 'background.' . strtolower(pathinfo($_FILES['background_site']['name'], PATHINFO_EXTENSION)); // On met en nom background et l'extension
    $dateArticle = ''; // Pas besoin de date pour les profils
}
...
if (!isset($parametre_upload_image)) {
    ...
    if (isset($_POST['nom'])) { // Création d'autre répertoire
        ...
        if (!file_exists("Jeux/" . $date . $url . '/' . $type_image)) { // Pas besoin de mettre la date
            mkdir("Jeux/" . $date . $url . '/' . $type_image, 0777, true);
        }
    } else if (isset($_POST['titre'])) {
        ...
        if (!file_exists("Articles/" . '/' . $date . $url . '/' . $type_image)) {
            mkdir("Articles/" . '/' . $date . $url . '/' . $type_image, 0777, true);
        }
    } else if (isset($_POST['image'])) {
        // $date = $annee . '/' . $mois . '/' . $jour . '/' . $heure . 'h' . $minute; // Date article qui va servir à créer les dossiers
    ...
    ?>
    <script>
    alert('<?php echo $categorie_article . $date . $titre_article . $type_image . '/' . $nom_image ?>');
    </script>
    <?php */
}
