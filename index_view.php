    ...
    <div class="view carousel-view d-none d-lg-block"> <!-- style="background-image: url('https://images.unsplash.com/photo-1563625236407-04ec2b4539d0?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjExMDk0fQ&auto=format&fit=crop&w=1916&q=80'); background-repeat: no-repeat; background-size: cover; background-position: center center;"> -->
        ...
        <div id="carousel" class="carousel slide " data-ride="carousel" data-interval="5000" style="width:100%;"> <!-- style="width:100%; object-fit: contain; height: 51vh; top:20px;"> -->
            ...
            <div class="carousel-inner" style="height:510px;">
                <?php $reponse = $bdd->prepare('SELECT article.id, article.titre, article.url, article.contenu, article.nom_banniere, carousel.page, DATE_FORMAT(date_creation, "%Y/%M/%d/%kh%i") AS date_article_dossier FROM article INNER JOIN carousel ON article.id = carousel.id_news ORDER BY page'); // Sélection des news du carousel
                $reponse->execute();
                    ...
                    if ($donnees['page'] == "1") { // Seule la première page est active au début
                     ?>
                        ...
                        <?php } else { ?>
                            ...
                            <img src="/Articles/<?php echo htmlspecialchars($donnees['date_article_dossier']); ?>/<?php echo htmlspecialchars($donnees['url']); ?>/bandeaux/<?php echo htmlspecialchars($donnees['nom_banniere']); ?>" onerror="this.oneerror=null; this.src='banniere.jpg';" class="img-fluid" style="max-height: 510px; height:auto; width: 100%;"> <!-- style="max-width: 230vh; min-height: 20vh; max-height: 43vh;"> --> <!-- object-fit : contain -->
